...
output_string_list:TStringlist; //Список для вывода в файл
output_file_path:string; //Путь для файла
...
//Прочитать файл настроек и установить свойства формы
...
MessageBox(0, PChar('Ошибка. Файл настроек (settings.xml) не найден.'),PChar('Сообщение'),MB_OK);
end;
...
begin
timer_seconds_count:=0; //Счетчик секунд
set_form_settings();
...
//Если процессы запущены - отображаем страницу в браузере
...
//Процессы должны запустится менее чем за 15 секунд (таймер остановится)иначе выводим сообщение
if (timer_seconds_count>15) then
begin
Preloader.Visible:=false;
form1.Preloader.Destroy;
Timer1.Enabled:=false;
MessageBox(0, PChar('Ошибка запуска программы. Программа не смогла запустить процессы caddy.exe и/или php-cgi.exe. Попробуйте запустить их вручную из папки программы и узнать причину ошибки. Возможно, поможет установка распространяемого пакета Visual C++ (vc_redist.x64)'),PChar('Ошибка'),MB_OK);
end;
...
begin
//Останавливаем сервер (убиваем все процессы связанные с сервером и его службами)
